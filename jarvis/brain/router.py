            ...
            if len(parts) > 1:
                ...
                if target in WEBSITE_ALIASES:
                    return {
                        "type": "tool",
                        "tool": "open_website",
                        "arguments": {"url": WEBSITE_ALIASES[target]}
                    }

                # allow 'open youtube' or 'open youtube.com'
                if target.rstrip('/').lower() in WEBSITE_ALIASES:
                    return {
                        "type": "tool",
                        "tool": "open_website",
                        "arguments": {"url": WEBSITE_ALIASES[target.rstrip('/').lower()]}
                    }
